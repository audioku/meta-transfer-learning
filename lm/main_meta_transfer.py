...
parser.add_argument('--meta_lr_factor', type=float, default=3,
                    help='')
...
log_name = str(args.name) + "_model" + str(args.model) + "_bptt" + str(args.bptt) + "_lr" + str(args.lr) + "_drop" + str(args.dropout) + "_layers" + str(args.nlayers) + "_nhid" + str(args.nhid) + "_emsize" + str(args.emsize) + "_meta_lr_factor" + str(args.meta_lr_factor) + "_ratio" + str(args.ratio) + ".txt"
...
def train(tr_data_source, val_data_source, start_it, num_it, log_interval, valid_interval):
    ...
    while it < num_it:
        ...
        for i in range(len(tr_data_source.task_list)):
            ...
            # if i == len(tr_data_source.task_list) - 1:
            #     new_hidden = repackage_hidden(new_hidden)
            #     hidden = deepcopy(new_hidden)
            ...
            inner_opt = optim.SGD(model.parameters(), lr=lr/args.meta_lr_factor)
            inner_opt.step()

            # Meta Validation
            output, _ = forward_one_batch(model, hidden, val_inputs)
            val_loss = criterion(output.view(-1, ntokens), val_targets)

            # outer loop optimization
            # batch_loss += val_loss / len(tr_data_source.task_list)

            if i < 2:
                batch_loss += val_loss * (1-args.ratio)/2
            else:
                batch_loss += val_loss * args.ratio
